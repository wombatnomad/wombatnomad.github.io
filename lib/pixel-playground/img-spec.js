...
/**
 * @typedef {{
 *   color: [number, number, number],    // currently active brush color
...
const parser = new AParser({
    ...
    rules: [
        ...
        {
            lhs: '#stmt', rhs: ['fill', '#color'], f: (_, color) => stmtf((scope, bmp) => {
        ...
        {
            lhs: '#stmt', rhs: ['color', '#color'], f: (_, color) => stmtf((scope, bmp) => {
                scope.color = color;
            })
        },
        {
