...
import assert from "../stable/assert.js";
import { eqNumberArray, ExplicitEquals } from "../stable/ops.js";
...
export class Matrix {
    ...
    *rows() {
     ...
     */
    _checkDim(other, R, C) {
        assert.xeq(other.R, R || this.R, 'Mismatched Matrix dimension (R)');
        assert.xeq(other.C, C || this.C, 'Mismatched Matrix dimension (C)');
    }

    /**
     * Create a new matrix by combining two matrices pairwise
     * @param {Matrix} other
    ...
    _pairMap(other, f) {
        ...
        for (let i = 0; i < values.length; i++) {
            values[i] = f(this.values[i], other.values[i]);
        }
        return new Matrix(this.R, this.C, values);
    }
     ...
     */
    add(other) {
        return this._pairMap(other, (a, b) => a + b);
    }
     ...
     */
    scale(value) {
        return this._elementMap(x => x * value);
    }
